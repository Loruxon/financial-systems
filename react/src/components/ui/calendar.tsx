  ...
  return (
    <DayPicker
      ...
      components={{
        ...
        // Сб/Вс — более приглушённый цвет, чтобы выходные визуально
        // отличались от будних при беглом взгляде на сетку.
        Weekday: ({ children, className, ...props }) => {
          const isWeekend = typeof children === "string" && ["сб", "вс"].includes(children.toLowerCase())
          return (
            <th className={cn(className, isWeekend && "text-muted-foreground/50")} {...props}>
              {children}
            </th>
          )
        },
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      data-hover-range={modifiers.hoverRange}
      className={cn(
        ...
        "data-[hover-range=true]:rounded-none data-[hover-range=true]:bg-primary/5",
        ...
        "data-[today=true]:ring-[1.5px] data-[today=true]:ring-inset data-[today=true]:ring-foreground/40",
