...
export function LinkRequestDialog({ receipt, requests, onClose, onLinked }: {
  receipt: Receipt | null
  requests: RequestListItem[]
  onClose: () => void
  onLinked: (updated: Receipt) => void
}) {
  const [selectedIds, setSelectedIds] = useState<number[]>([])
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    if (receipt) setSelectedIds(receipt.requests)
  }, [receipt])

  const filteredRequests = receipt?.organization_id
    ? requests.filter((r) => r.organization_id === receipt.organization_id)
    : requests

  const receiptAmount = receipt ? parseFloat(receipt.amount) : null
  const matchingRequests = filteredRequests.filter(
    (r) => r.prf_amount !== null && receiptAmount !== null && parseFloat(r.prf_amount) === receiptAmount
  )
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      onLinked(await api.setAdminIncomingPaymentRequests(receipt.id, selectedIds))
      onClose()
    } catch {
            ...
            <div className="rounded-xl border overflow-hidden">
              ...
              <div className="px-4 pt-3 pb-4 flex items-center justify-between gap-4">
                <div className="flex flex-col gap-0.5 min-w-0">
                  ...
                  <span className="text-sm font-semibold truncate">{receipt.recipient_name ?? "—"}</span>
                </div>
                <div className="flex flex-col gap-0.5 items-end shrink-0">
                  <span className="text-xs text-muted-foreground tabular-nums">
                    {receipt.date.split("-").reverse().join(".")}
                  </span>
                  <span className="text-base font-bold tabular-nums text-primary">
                    {fmtNum(parseFloat(receipt.amount))} ₽
                  </span>
                </div>
              </div>
            </div>
