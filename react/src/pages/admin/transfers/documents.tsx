...
import { api, type TransferDocumentItem } from "@/lib/api"
...
// Плоский список документов перевода — тот же паттерн, что и у исходящих
// платежей (outgoing-payments/documents.tsx): без раздела/типа документа.

export interface TransferDocumentsHandle {
  getCount: () => number
  /** Реально загружает файлы, накопленные в буфере (пока перевода не было),
   *  на переданный id — вызывается родителем сразу после создания перевода. */
  flush: (transferId: number) => Promise<void>
}
...
export const TransferDocuments = forwardRef<TransferDocumentsHandle, { transferId?: number }>(
  function TransferDocuments({ transferId }, ref) {
    const inputRef = useRef<HTMLInputElement>(null)
    const [isDragging, setIsDragging] = useState(false)
    const [items, setItems] = useState<TransferDocumentItem[]>([])
    const [pending, setPending] = useState<PendingUpload[]>([])
    // Файлы, выбранные до создания перевода — реально грузятся только через
    // flush(), сразу после того, как перевод сохранён и получил id.
    const [buffered, setBuffered] = useState<BufferedFile[]>([])

    useEffect(() => {
      if (!transferId) return
      api.getAdminTransferDocuments(transferId)
        .then(setItems)
    ...
    const uploadNow = async (id: number, files: File[]) => {
      ...
      await Promise.all(files.map(async (f, i) => {
        ...
        try {
          const doc = await api.uploadAdminTransferDocument(id, f)
    ...
    const addFiles = async (files: File[]) => {
      ...
      await uploadNow(transferId, files)
    }
    ...
    const removeItem = async (id: number) => {
      try {
        await api.deleteAdminTransferDocument(id)
